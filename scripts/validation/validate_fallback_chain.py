...
async def main():
    ...
    result_a = await wds.fetch_wallet_swap_history(
        client, env, "WALLET_A", sol_price_usd=150.0, connection=None
    )
    ...
    wds.get_wallet_recent_transactions = (
        fake_get_wallet_recent_transactions  # monkeypatch the real module's binding
    )
    ...
    print(
        f"[PASS] Scenario B: Helius 429 -> RPC fallback reconstructs {len(result_b.swaps)} swap(s), "
    ...
    result_c = await wds.fetch_wallet_swap_history(
        client, env, "WALLET_B", sol_price_usd=150.0, connection=object()
    )
    ...
    print(
        f"[PASS] Scenario C: same wallet re-fetched -> served from stale cache (source={result_c.source}), "
