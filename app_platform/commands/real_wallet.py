...
class RealWalletStates(StatesGroup):
    ...
    waiting_trailing_activation_pct = State()
    waiting_trailing_trail_pct = State()
    waiting_trailing_initial_sl_pct = State()
...
async def _show_auto_filters_panel(target, user_id: int, edit: bool):
    ...
    text = (
        ...
        f"📉 <b>Trailing Stop</b> (auto-buys): <b>{'ON' if filt.trailing_stop_enabled else 'OFF'}</b>\n"
        f"📈 Trailing Activation %: <b>{filt.trailing_activation_pct if filt.trailing_activation_pct is not None else f'default ({real_trailing_stop_engine.DEFAULT_ACTIVATION_PCT:g})'}</b>\n"
        f"📉 Trailing %: <b>{filt.trailing_pct if filt.trailing_pct is not None else f'default ({real_trailing_stop_engine.DEFAULT_TRAIL_PCT:g})'}</b>\n"
        f"🛑 Initial Stop Loss %: <b>{filt.trailing_initial_stop_loss_pct if filt.trailing_initial_stop_loss_pct is not None else '—'}</b>\n"
        f"🪜 Trailing Step %: <b>{filt.trailing_step_pct if filt.trailing_step_pct is not None else f'default ({real_trailing_stop_engine.DEFAULT_TRAILING_STEP_PCT:g})'}</b>\n\n"
        "Tap a field to edit it."
    )
    kb = real_wallet_automation_filters_kb(filt.auto_buy_signal_source or "both", bool(filt.trailing_stop_enabled))
    if edit:
        await target.edit_text(text, reply_markup=kb)
    else:
        await target.answer(text, reply_markup=kb)
...
_FILTER_FIELD_LABELS = {
    ...
    "trailing_activation_pct": "Trailing Stop activation % (gain from entry required to arm), e.g. 10",
    "trailing_pct": "Trailing Stop retracement %, e.g. 15 for a 15% pullback from the peak",
    "trailing_initial_stop_loss_pct": "Trailing Stop's initial floor % before it arms, e.g. 20",
    "trailing_step_pct": "Trailing Stop step %, the minimum peak improvement before the stop moves up, e.g. 1",
}
...
@router.callback_query(F.data == "rw:trail_default_toggle")
async def cb_trail_default_toggle(callback: CallbackQuery):
    filt = await real_automation_engine.get_or_create_filter(callback.from_user.id)
    new_state = not bool(filt.trailing_stop_enabled)
    ok = await real_automation_engine.update_filter(callback.from_user.id, "trailing_stop_enabled", new_state)
    if not ok:
        await callback.answer("❌ Could not update that setting.", show_alert=True)
        return
    await _show_auto_filters_panel(callback.message, callback.from_user.id, edit=True)
    await callback.answer(f"Trailing Stop on auto-buys turned {'ON' if new_state else 'OFF'}")
...
# ---------------------------------------------------------------------------
# Trailing Stop — manual per-position attach/cancel (also auto-attached to
# Auto Buy on Signal positions from the Auto-Buy Filters defaults above)
# ---------------------------------------------------------------------------

def _format_trailing_status(ts) -> str:
    if ts is None or ts.status not in ("watching", "armed"):
        return "No trailing stop on this position yet."
    lines = [
        f"Activation: +{ts.activation_pct:g}% from entry",
        f"Trailing: {ts.trail_pct:g}% from peak",
    ]
    if ts.initial_stop_loss_pct:
        lines.append(f"Initial Stop Loss: -{ts.initial_stop_loss_pct:g}% from entry (until armed)")
    if ts.status == "armed":
        lines.append(f"Status: 🟢 ARMED — peak {ts.highest_price_seen:.8f}, stop at {ts.current_stop_price:.8f}")
    else:
        lines.append("Status: ⏳ Watching (not yet armed)")
    return "\n".join(lines)


@router.callback_query(F.data.startswith("rw:trail_menu:"))
async def cb_trail_menu(callback: CallbackQuery):
    trade_id = int(callback.data.split(":")[-1])
    ts = await real_trailing_stop_engine.get_trailing_stop_for_trade(callback.from_user.id, trade_id)
    text = f"📉 <b>Trailing Stop</b>\n\n{_format_trailing_status(ts)}"
    await callback.message.edit_text(text, reply_markup=real_wallet_trailing_menu_kb(trade_id, ts))
    await callback.answer()


@router.callback_query(F.data.startswith("rw:trail_add:"))
async def cb_trail_add(callback: CallbackQuery, state: FSMContext):
    trade_id = int(callback.data.split(":")[-1])
    await state.set_state(RealWalletStates.waiting_trailing_activation_pct)
    await state.update_data(trail_trade_id=trade_id)
    await callback.message.edit_text(
        "✏️ <b>Trailing Stop — Activation %</b>\n\n"
        "How many % should price rise from entry before the trailing stop arms? "
        "(e.g. <code>10</code> for +10%. Send <code>0</code> to arm immediately.)\n\n"
        "Send /cancel to back out."
    )
    await callback.answer()


@router.message(RealWalletStates.waiting_trailing_activation_pct)
async def on_trailing_activation_pct_message(message: Message, state: FSMContext):
    raw = (message.text or "").strip()
    if raw.lower() == "/cancel":
        await state.clear()
        await message.answer("Cancelled.")
        return
    try:
        activation_pct = float(raw)
        if activation_pct < 0:
            raise ValueError
    except ValueError:
        await message.answer("❌ Enter a number 0 or greater, e.g. <code>10</code>, or /cancel.")
        return

    await state.update_data(trail_activation_pct=activation_pct)
    await state.set_state(RealWalletStates.waiting_trailing_trail_pct)
    await message.answer(
        "✏️ <b>Trailing Stop — Trailing %</b>\n\n"
        "Once armed, how many % should price be allowed to pull back from its peak before selling? "
        "(e.g. <code>15</code> for a 15% pullback)\n\nSend /cancel to back out."
    )


@router.message(RealWalletStates.waiting_trailing_trail_pct)
async def on_trailing_trail_pct_message(message: Message, state: FSMContext):
    raw = (message.text or "").strip()
    if raw.lower() == "/cancel":
        await state.clear()
        await message.answer("Cancelled.")
    ...
    try:
        ...
        await message.answer("❌ Enter a positive number, e.g. <code>15</code>, or /cancel.")
        return

    await state.update_data(trail_trail_pct=trail_pct)
    await state.set_state(RealWalletStates.waiting_trailing_initial_sl_pct)
    await message.answer(
        "✏️ <b>Trailing Stop — Initial Stop Loss %</b> (optional)\n\n"
        "Protect the position with a fixed stop-loss % from entry before the trailing "
        "stop arms? Send a number (e.g. <code>20</code>), or <code>skip</code> for no initial floor.\n\n"
        "Send /cancel to back out."
    )


@router.message(RealWalletStates.waiting_trailing_initial_sl_pct)
async def on_trailing_initial_sl_pct_message(message: Message, state: FSMContext):
    raw = (message.text or "").strip()
    if raw.lower() == "/cancel":
        await state.clear()
        await message.answer("Cancelled.")
    ...
    if raw.lower() != "skip":
        try:
            initial_sl_pct = float(raw)
            if initial_sl_pct <= 0:
                raise ValueError
        except ValueError:
            await message.answer("❌ Enter a positive number, <code>skip</code>, or /cancel.")
            return

    data = await state.get_data()
    trade_id = data.get("trail_trade_id")
    activation_pct = data.get("trail_activation_pct")
    trail_pct = data.get("trail_trail_pct")
    await state.clear()
    if trade_id is None or activation_pct is None or trail_pct is None:
        await message.answer("❌ Lost track of this setup — open Trailing Stop again from the position.")
        return

    try:
        await real_trailing_stop_engine.create_trailing_stop(
    ...
    except real_trailing_stop_engine.TrailingStopValidationError as e:
        await message.answer(f"❌ {html.escape(str(e))}")
        return

    ts = await real_trailing_stop_engine.get_trailing_stop_for_trade(message.from_user.id, trade_id)
    await message.answer(
        f"✅ Trailing Stop set: arms at +{activation_pct:g}%, trails {trail_pct:g}% from peak"
        f"{f', initial floor -{initial_sl_pct:g}%' if initial_sl_pct else ''}.",
        reply_markup=real_wallet_trailing_menu_kb(trade_id, ts),
    )


@router.callback_query(F.data.startswith("rw:trail_cancel:"))
async def cb_trail_cancel(callback: CallbackQuery):
    trade_id = int(callback.data.split(":")[-1])
    ok = await real_trailing_stop_engine.cancel_trailing_stop(callback.from_user.id, trade_id)
    if not ok:
        await callback.answer("No active trailing stop on this position.", show_alert=True)
        return
    ts = await real_trailing_stop_engine.get_trailing_stop_for_trade(callback.from_user.id, trade_id)
    await callback.message.edit_text(
        f"📉 <b>Trailing Stop</b>\n\n{_format_trailing_status(ts)}",
