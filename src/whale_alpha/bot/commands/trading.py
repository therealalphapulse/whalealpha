"""Trading status and controls for Whale Alpha."""
...
def register_trading_commands(session_factory: async_sessionmaker[AsyncSession]) -> Router:
    ...
    async def portfolio_handler(message: Message) -> None:
        ...
        async with session_factory() as session:
            ...
            recent_trades = list(trades_result.scalars())

        if not recent_trades:
            await message.answer("No trades yet. Wait for a signal or use /buy for a manual entry.")
            return
        lines = [f"{t.side.value} {t.token_mint[:6]}... — ${t.amount_usd:.2f} ({t.source.value})" for t in recent_trades]
    ...
    async def autotrading_handler(message: Message) -> None:
        ...
        async with session_factory() as session:
            result = await session.execute(
                select(User).options(selectinload(User.auto_trading_config)).where(User.telegram_id == telegram_id)
            )
            user = result.scalar_one_or_none()
            if user is None:
                await message.answer("Use /start first to initialize your account.")
                return
            enabled = bool(user.auto_trading_config and user.auto_trading_config.enabled)

        await message.answer(
            f"🤖 *Auto Buy*: {'ENABLED ✅' if enabled else 'disabled'}\n"
