...
def _milestone_text(opportunity: TokenOpportunity, milestone_pct: int, current_price: float) -> str:
    ...
    return (
        f"🚀 <b>${opportunity.symbol or 'UNKNOWN'} reached {milestone_pct}%</b>\n\n"
        f"🔥 From initial signal: <b>+{pct:.1f}%</b>\n"
        f"💎 Multiple: <b>{multiple:.2f}x</b>\n\n"
        f"💰 Signal price: <b>${reference:.10g}</b>\n"
        f"📈 Current price: <b>${current_price:.10g}</b>\n\n"
        f"<code>{opportunity.mint}</code>\n\n"
        f"<i>Whale Alpha • Milestone Alert</i>"
    )
...
async def _check_signal_milestones(
    ...
    mints = [o.mint for o in opportunities if (o.alert_reference_price_usd or 0) > 0 and o.alert_message_ids]
    if not mints:
        return
    ...
    async with session_factory() as session:
        for opportunity in opportunities:
            ...
            if reference <= 0 or not message_ids:
                continue
            ...
            sent = set(int(x) for x in (opportunity.quote_milestones or []))
            changed = False
            for milestone_pct in crossed_milestones(current_pct, sent):
