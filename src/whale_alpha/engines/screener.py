...
from whale_alpha.integrations.token_hunter_sources import discover_token_candidates
...
async def run_screener_cycle(
    ...
    for values in sources.values():
        for candidate in values:
            if candidate.snapshot.mint in seen:
                continue
            seen.add(candidate.snapshot.mint)
    ...
    regime = classify_market_regime([c.snapshot for c in prequalified]) if env.TOKEN_HUNTER_MARKET_REGIME_ENABLED else None
    log.info(
        "screener_market_regime",
        name=regime.name if regime else "DISABLED",
