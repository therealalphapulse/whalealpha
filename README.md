...
| Whale Wallet Discovery & Intelligence Engine | `engines/discovery.py`, `engines/discovery_metrics.py`, `integrations/wallet_discovery_source.py` | Fully automated: sources candidates from other large holders of recently-signaled tokens, scores them with the unchanged `engines/scoring.py` algorithm, and auto-promotes/retires wallets to keep the tracked population between `DISCOVERY_MIN_TRACKED_WALLETS` and `DISCOVERY_MAX_TRACKED_WALLETS` (default 500–1500). Users still can never add wallets — this is the automated-admin path, gated behind the same `WhaleWalletAdminService` RBAC choke point as `/addwhale`. New `WalletCandidate` table (migration `0003_...`). Realized-PnL history requires `HELIUS_API_KEY`; without it, candidate sourcing and inactivity-based retirement still run, but score refresh is skipped — see that module's docstring. |
