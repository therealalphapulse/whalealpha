...
| Whale Wallet Discovery & Intelligence Engine | `engines/discovery.py`, `engines/discovery_metrics.py`, `integrations/wallet_discovery_source.py` | Fully automated: sources candidates from two independent streams — holders of tokens your own tracked whales recently signaled, plus Jupiter's platform-wide trending tokens (so it can find its first wallets from a completely empty database, no admin seeding required) — scores them with the unchanged `engines/scoring.py` algorithm, and auto-promotes/retires wallets to keep the tracked population between `DISCOVERY_MIN_TRACKED_WALLETS` and `DISCOVERY_MAX_TRACKED_WALLETS` (default 500–1500). Users still can never add wallets — this is the automated-admin path, gated behind the same `WhaleWalletAdminService` RBAC choke point as `/addwhale`. New `WalletCandidate` table (migration `0003_...`). Needs `JUPITER_API_KEY`/`PRICE_FEED_API_KEY` for the trending-token bootstrap source and `HELIUS_API_KEY` for scoring — see `docs/ARCHITECTURE.md`'s "Two independent API keys" note for what each does and doesn't gate. |
